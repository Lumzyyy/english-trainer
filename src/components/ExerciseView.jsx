...
function Badge({ label, color, bg }) {
  return <span style={{ display: 'inline-flex', alignItems: 'center', fontSize: '.7rem', fontWeight: 600, padding: '2px 8px', borderRadius: 20, background: bg, color, marginRight: 6, letterSpacing: '.03em' }}>{label}</span>
}
...
function ExerciseCard({ ex, idx, total, onNext, onPrev }) {
  ...
  const isCorrect = () => {
    ...
    // Allow partial match for fill (accepts main answer separated by /)
    const variants = expected.split('/').map(v => v.trim())
    return variants.some(v => ans === v || ans.includes(v) || v.includes(ans))
  }
  ...
  return (
    <div className="card" style={{ padding: 20 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16, flexWrap: 'wrap' }}>
        <span style={{ fontFamily: 'var(--font-title)', fontSize: '.75rem', color: 'var(--text3)' }}>{idx + 1}/{total}</span>
      ...
      {ex.type === 'choose' ? (
        ...
        <input type="text" disabled={submitted} placeholder={ex.type === 'translate' ? 'Ta traduction...' : ex.type === 'correct' ? 'Ta correction (ou ✓ si c\'est correct)...' : 'Ta réponse...'}
      ...
      {submitted && (
        <div style={{ borderRadius: 8, padding: '12px 14px', marginBottom: 16, background: correct ? 'var(--success-bg)' : 'var(--error-bg)', border: `1px solid ${correct ? 'var(--success)' : 'var(--error)'}` }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: correct ? 0 : 8 }}>
            {correct
          ...
          {!correct && (
            <div style={{ marginBottom: 10, padding: '8px 12px', background: 'var(--surface)', borderRadius: 6 }}>
              <div style={{ fontSize: '.75rem', color: 'var(--text3)', marginBottom: 4 }}>Ta réponse</div>
              <div style={{ fontFamily: 'var(--font-title)', fontSize: '.875rem', color: 'var(--error)' }}>{ex.type === 'choose' ? selected : input}</div>
              <div style={{ fontSize: '.75rem', color: 'var(--text3)', marginTop: 8, marginBottom: 4 }}>Bonne réponse</div>
...
export default function ExerciseView({ exercises }) {
  ...
  const filtered = useMemo(() =>
    ...
    ), [exercises, levelFilter, contextFilter])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
        <button className="btn btn-ghost" style={{ fontSize: '.8rem', padding: '6px 10px' }} onClick={() => setShowFilters(s => !s)}>
        ...
        <span style={{ fontFamily: 'var(--font-title)', fontSize: '.75rem', color: 'var(--text3)' }}>{safeIdx + 1}/{filtered.length}</span>
      </div>

      {showFilters && (
        <div className="card" style={{ padding: '12px 16px', display: 'flex', gap: 12, flexWrap: 'wrap' }}>
          <div>
            <div style={{ fontSize: '.7rem', color: 'var(--text3)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.05em' }}>Niveau</div>
            <div style={{ display: 'flex', gap: 6 }}>
              {['all', 'easy', 'medium', 'hard'].map(l => (
                <button key={l} className={`btn ${levelFilter === l ? 'btn-primary' : 'btn-ghost'}`} style={{ fontSize: '.75rem', padding: '4px 10px' }}
          ...
          <div>
            <div style={{ fontSize: '.7rem', color: 'var(--text3)', marginBottom: 6, textTransform: 'uppercase', letterSpacing: '.05em' }}>Contexte</div>
            <div style={{ display: 'flex', gap: 6 }}>
              {['all', 'daily', 'data'].map(c => (
                <button key={c} className={`btn ${contextFilter === c ? 'btn-primary' : 'btn-ghost'}`} style={{ fontSize: '.75rem', padding: '4px 10px' }}
                  onClick={() => { setContextFilter(c); setIdx(0) }}>
                  {c === 'all' ? 'Tous' : c === 'daily' ? '☀️ Quotidien' : '⚡ Data/IA'}
                </button>
              ))}
            </div>
          </div>
          <button className="btn btn-ghost" style={{ alignSelf: 'flex-end', fontSize: '.75rem', padding: '4px 10px' }}
            onClick={() => { setIdx(0) }}>
      ...
      <ExerciseCard key={`${current.id}-${safeIdx}`} ex={current} idx={safeIdx} total={filtered.length}
